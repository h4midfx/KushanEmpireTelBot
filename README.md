...
<div id="language-select" style="text-align:center; margin-bottom:10px;">
  ...
  <button onclick="setLang('fa')"><img src="https://flagcdn.com/ir.svg" width="25" alt="Farsi Flag" /> فارسی</button>
</div>

<h1 id="page-title">🏰 Kushan Empire</h1>
...
const texts = {
  ...
  fa: {
    ...
    delete: "حذف",
    searchPlaceholder: "جستجو برای مهاجم، هدف یا اتحاد...",
    newestFirst: "جدیدترین‌ها ابتدا",
    oldestFirst: "قدیمی‌ترین‌ها ابتدا",
    adminLogin: "🔑 ورود مدیر",
    manageNotes: "📝 مدیریت یادداشت‌ها",
    noteTitle: "مدیریت یادداشت مدیر",
    noteLabelText: "متن یادداشت:",
    noteLabelFrom: "نمایش یادداشت از (تاریخ/زمان):",
    noteLabelTo: "نمایش یادداشت تا (تاریخ/زمان):",
    noteSaveBtn: "ذخیره یادداشت",
    cancelBtn: "انصراف",
    selectSectionPrompt: "انتخاب بخش:",
    passwordPrompt: "رمز عبور بخش کوشان 1 را وارد کنید:"
  }
};
...
function datenAnzeigen() {
  ...
  inhalt.innerHTML = `<h2>${texts[currentLang].viewData} – ${currentSection.toUpperCase()}</h2>`;
...
function showNotesIfVisible() {
  // Diese Funktion prüft, ob wir eine gültige Notiz haben, die jetzt angezeigt werden soll
  db.ref("adminNotes").once("value", snapshot => {
    const note = snapshot.val();
...
window.onload = () => {
  ...
  showNotesIfVisible();
  // Falls gewünscht, kann man ein Interval setzen, um Notizen automatisch zu prüfen:
  setInterval(showNotesIfVisible, 60 * 1000); // alle 60 Sekunden prüfen
};
